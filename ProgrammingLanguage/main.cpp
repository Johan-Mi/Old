...
struct Rvalue {
	...
	Rvalue(const Lvalue& lval) : type(lval.value->type),
	value(lval.value->value) {}
...
constexpr auto lexerDelimiters
= std::make_array<std::string_view>(";", "+", "-", "*", "/", "=", "!", "(",
		")", "{", "}", "[", "]");
constexpr auto lexerSplitters
= std::make_array<std::string_view>(" ");
constexpr std::string_view				   lexerToggler = "\"";

enum struct TokenType {
	Number, BOperator, UOperator, Declaration, Statement, Assignment, Other
};

constexpr auto parserBOperators
= std::make_array<std::string_view>("+", "-", "*", "/");
constexpr auto parserUOperators
= std::make_array<std::string_view>("!");
...
int main() {
	...
	std::vector<std::string> lexedProgram = Lex(rawProgram,
			lexerTokens, lexerDelimiters, lexerSplitters, lexerToggler);
