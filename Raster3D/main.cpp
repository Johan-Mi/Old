...
struct Line3D { Point3D p1, p2; };
struct Tri2D { Point2D p1, p2, p3; olc::Pixel colour = olc::WHITE; };
struct Tri3D { Point3D p1, p2, p3; olc::Pixel colour = olc::WHITE; };
...
auto pointToScreen = [](Point3D p) {return Point2D{p.x / p.z, p.y / p.z}; };
inline void addTri(Tri2D t, float depth) { storedTris.push_back({t, depth}); }
...
Point3D pointCameraTransform(Point3D p, Camera c) {
	...
	p.z -= c.z;
	Point3D midpoint{
		p.x * cosf(c.yaw) + p.z * sinf(c.yaw),
		p.y,
		-p.x * sinf(c.yaw) + p.z * cosf(c.yaw)
	};
	return Point3D{
		midpoint.x,
		midpoint.y * cosf(c.pitch) + midpoint.z * sinf(c.pitch),
...
Point3D nearPlaneLineIntersection(Point3D inP1, Point3D inP2) {
	...
	return Point3D{
		inP1.x + t * (inP2.x - inP1.x),
		inP1.y + t * (inP2.y - inP1.y),
		fMinDrawDistance
	};
}
...
public:
	bool pointOnScreen(Point2D p) { return p.x > 0 && p.x < ScreenWidth() && p.y > 0 && p.y < ScreenHeight(); }
	void renderPoint(Point2D p) { DrawCircle(ScreenWidth() / 2 - 200 * p.x, ScreenHeight() / 2 - 200 * p.y, 2, olc::WHITE); }
	void renderLine(Point2D p1, Point2D p2) { DrawLine(ScreenWidth() / 2 - 200 * p1.x, ScreenHeight() / 2 - 200 * p1.y, ScreenWidth() / 2 - 200 * p2.x, ScreenHeight() / 2 - 200 * p2.y, olc::WHITE); }

	void renderClippedTri(Tri2D t) {
		for(auto k : triClipDown(triClipUp(triClipRight(triClipLeft(std::vector<Tri2D>{Tri2D{
			Point2D{ScreenWidth() / 2 - 200 * t.p1.x, ScreenHeight() / 2 - 200 * t.p1.y},
			Point2D{ScreenWidth() / 2 - 200 * t.p2.x, ScreenHeight() / 2 - 200 * t.p2.y},
			Point2D{ScreenWidth() / 2 - 200 * t.p3.x, ScreenHeight() / 2 - 200 * t.p3.y},
			t.colour
			}})))))
			FillTriangle(k.p1.x, k.p1.y, k.p2.x, k.p2.y, k.p3.x, k.p3.y, k.colour);
	}

	Point2D lineIntersect2D(Point2D inP1, Point2D inP2, bool bAxis, bool bSide) { return bAxis ? (bSide ? Point2D{inP1.x + ((float)ScreenHeight() - inP1.y) * (inP2.x - inP1.x) / (inP2.y - inP1.y), (float)ScreenHeight()} : Point2D{inP1.x - inP1.y * (inP2.x - inP1.x) / (inP2.y - inP1.y), 0}) : (bSide ? Point2D{(float)ScreenWidth(), inP1.y + ((float)ScreenWidth() - inP1.x) * (inP2.y - inP1.y) / (inP2.x - inP1.x)} : Point2D{0, inP1.y - inP1.x * (inP2.y - inP1.y) / (inP2.x - inP1.x)}); }
	...
	std::vector<Tri2D> triClipLeft(std::vector<Tri2D> inTris) {
		...
		for(auto h : inTris) {
			...
			case 1:
				...
				*backPoint1 = lineIntersect2D(*backPoint1, *frontPoint, false, false);
				*backPoint2 = lineIntersect2D(*backPoint2, *frontPoint, false, false);
				outTris.push_back(Tri2D{p1, p2, p3, h.colour});
				break;
			...
			case 2:
				...
				Point2D backPointDraw1 = lineIntersect2D(*backPoint, *frontPoint1, false, false);
				Point2D backPointDraw2 = lineIntersect2D(*backPoint, *frontPoint2, false, false);
				outTris.push_back(Tri2D{*frontPoint1, *frontPoint2, backPointDraw1, h.colour});
				outTris.push_back(Tri2D{backPointDraw2, *frontPoint2, backPointDraw1, h.colour});
				break;
	...
	std::vector<Tri2D> triClipRight(std::vector<Tri2D> inTris) {
		...
		for(auto h : inTris) {
			...
			Point2D p2 = h.p2;
			Point2D p3 = h.p3;
			switch((p1.x <= ScreenWidth()) + (p2.x <= ScreenWidth()) + (p3.x <= ScreenWidth())) {
			case 1:
				Point2D* frontPoint;
				Point2D* backPoint1;
				Point2D* backPoint2;
				...
				*backPoint1 = lineIntersect2D(*backPoint1, *frontPoint, false, true);
				*backPoint2 = lineIntersect2D(*backPoint2, *frontPoint, false, true);
				outTris.push_back(Tri2D{p1, p2, p3, h.colour});
				break;
			...
			case 2:
				...
				Point2D backPointDraw1 = lineIntersect2D(*backPoint, *frontPoint1, false, true);
				Point2D backPointDraw2 = lineIntersect2D(*backPoint, *frontPoint2, false, true);
				outTris.push_back(Tri2D{*frontPoint1, *frontPoint2, backPointDraw1, h.colour});
				outTris.push_back(Tri2D{backPointDraw2, *frontPoint2, backPointDraw1, h.colour});
				break;
	...
	std::vector<Tri2D> triClipUp(std::vector<Tri2D> inTris) {
		...
		for(auto h : inTris) {
			...
			case 1:
				...
				*backPoint1 = lineIntersect2D(*backPoint1, *frontPoint, true, false);
				*backPoint2 = lineIntersect2D(*backPoint2, *frontPoint, true, false);
				outTris.push_back(Tri2D{p1, p2, p3, h.colour});
				break;
			...
			case 2:
				...
				Point2D backPointDraw1 = lineIntersect2D(*backPoint, *frontPoint1, true, false);
				Point2D backPointDraw2 = lineIntersect2D(*backPoint, *frontPoint2, true, false);
				outTris.push_back(Tri2D{*frontPoint1, *frontPoint2, backPointDraw1, h.colour});
				outTris.push_back(Tri2D{backPointDraw2, *frontPoint2, backPointDraw1, h.colour});
				break;
	...
	std::vector<Tri2D> triClipDown(std::vector<Tri2D> inTris) {
		...
		for(auto h : inTris) {
			...
			Point2D p2 = h.p2;
			Point2D p3 = h.p3;
			switch((p1.y <= ScreenHeight()) + (p2.y <= ScreenHeight()) + (p3.y <= ScreenHeight())) {
			case 1:
				Point2D* frontPoint;
				Point2D* backPoint1;
				Point2D* backPoint2;
				...
				*backPoint1 = lineIntersect2D(*backPoint1, *frontPoint, true, true);
				*backPoint2 = lineIntersect2D(*backPoint2, *frontPoint, true, true);
				outTris.push_back(Tri2D{p1, p2, p3, h.colour});
				break;
			...
			case 2:
				...
				Point2D backPointDraw1 = lineIntersect2D(*backPoint, *frontPoint1, true, true);
				Point2D backPointDraw2 = lineIntersect2D(*backPoint, *frontPoint2, true, true);
				outTris.push_back(Tri2D{*frontPoint1, *frontPoint2, backPointDraw1, h.colour});
				outTris.push_back(Tri2D{backPointDraw2, *frontPoint2, backPointDraw1, h.colour});
				break;
	...
	void triPipeline(Tri3D t) {
		...
		switch((p1.z >= fMinDrawDistance) + (p2.z >= fMinDrawDistance) + (p3.z >= fMinDrawDistance)) {
		case 1:
			Point3D* frontPoint;
			Point3D* backPoint1;
			Point3D* backPoint2;
			...
			*backPoint2 = nearPlaneLineIntersection(*backPoint2, *frontPoint);
			addTri(Tri2D{pointToScreen(p1), pointToScreen(p2), pointToScreen(p3), t.colour}, p1.z + p2.z + p3.z);
			break;
		...
		case 2:
			...
			Point3D backPointDraw1 = nearPlaneLineIntersection(*backPoint, *frontPoint1);
			Point3D backPointDraw2 = nearPlaneLineIntersection(*backPoint, *frontPoint2);
			addTri(Tri2D{pointToScreen(*frontPoint1), pointToScreen(*frontPoint2), pointToScreen(backPointDraw1), t.colour}, p1.z + p2.z + p3.z);
			addTri(Tri2D{pointToScreen(backPointDraw2), pointToScreen(*frontPoint2), pointToScreen(backPointDraw1), t.colour}, p1.z + p2.z + p3.z);
			break;
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if(bRenderTris) {
			...
			for(auto i : tris)
				triPipeline(i);
			std::sort(storedTris.begin(), storedTris.end(), [](std::pair<Tri2D, float> element1, std::pair<Tri2D, float> element2) {
