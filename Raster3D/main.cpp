...
struct Tri2D {
	Point2D p1, p2, p3; olc::Pixel colour = olc::WHITE;
};
...
class Example : public olc::PixelGameEngine {
	...
	public:
		...
		void renderPoint(Point2D p) {
			DrawCircle(ScreenWidth() / 2 - 200 * p.x,
					ScreenHeight() / 2 - 200 * p.y, 2, olc::WHITE);
		}
		void renderLine(Point2D p1, Point2D p2) {
			DrawLine(ScreenWidth() / 2 - 200 * p1.x,
					ScreenHeight() / 2 - 200 * p1.y,
					ScreenWidth() / 2 - 200 * p2.x,
					ScreenHeight() / 2 - 200 * p2.y, olc::WHITE);
		}

		void renderClippedTri(Tri2D t) {
			auto clippedTriangles = triClipDown(
					triClipUp(
						triClipRight(
							triClipLeft(
								std::vector<Tri2D>{{
								Point2D{ScreenWidth() / 2 - 200 * t.p1.x,
								ScreenHeight() / 2 - 200 * t.p1.y},
								Point2D{ScreenWidth() / 2 - 200 * t.p2.x,
								ScreenHeight() / 2 - 200 * t.p2.y},
								Point2D{ScreenWidth() / 2 - 200 * t.p3.x,
				...
				for(auto k : clippedTriangles)
					FillTriangle(k.p1.x, k.p1.y, k.p2.x,
							k.p2.y, k.p3.x, k.p3.y, k.colour);
		}

		Point2D lineIntersect2D(Point2D inP1, Point2D inP2,
				bool bAxis, bool bSide) {
			return bAxis
				? (bSide
						? Point2D{inP1.x + ((float)ScreenHeight() - inP1.y) *
						(inP2.x - inP1.x) / (inP2.y - inP1.y),
						(float)ScreenHeight()}
						: Point2D{inP1.x - inP1.y * (inP2.x - inP1.x) /
						(inP2.y - inP1.y), 0})
				: (bSide
						? Point2D{(float)ScreenWidth(), inP1.y +
						((float)ScreenWidth() - inP1.x) * (inP2.y - inP1.y) /
						(inP2.x - inP1.x)}
						: Point2D{0, inP1.y - inP1.x * (inP2.y - inP1.y) /
						(inP2.x - inP1.x)});
		}
					...
					case 1:
						{
							...
							*backPoint1 =
								lineIntersect2D(*backPoint1, *frontPoint,
										false, false);
							*backPoint2 =
								lineIntersect2D(*backPoint2, *frontPoint,
										false, false);
					...
					case 2:
						{
							...
							Point2D backPointDraw1 =
								lineIntersect2D(*backPoint, *frontPoint1,
										false, false);
							Point2D backPointDraw2 =
								lineIntersect2D(*backPoint, *frontPoint2,
										false, false);
							outTris.emplace_back(*frontPoint1,
									*frontPoint2, backPointDraw1, h.colour);
							outTris.emplace_back(backPointDraw2,
									*frontPoint2, backPointDraw1, h.colour);
						}
						break;
		...
		void triPipeline(Tri3D t) {
			...
			switch((p1.z >= fMinDrawDistance)
					+ (p2.z >= fMinDrawDistance)
				...
				case 1:
					{
						...
						*backPoint1 =
							nearPlaneLineIntersection(*backPoint1, *frontPoint);
						*backPoint2 =
							nearPlaneLineIntersection(*backPoint2, *frontPoint);
						addTri(Tri2D{pointToScreen(p1),
								pointToScreen(p2),
								pointToScreen(p3),
								t.colour}, p1.z + p2.z + p3.z);
					}
					break;
				...
				case 2:
					{
						...
						addTri(Tri2D{pointToScreen(*frontPoint1),
								pointToScreen(*frontPoint2),
								pointToScreen(backPointDraw1), t.colour},
								p1.z + p2.z + p3.z);
						addTri(Tri2D{pointToScreen(backPointDraw2),
								pointToScreen(*frontPoint2),
								pointToScreen(backPointDraw1),
								t.colour}, p1.z + p2.z + p3.z);
					}
					break;
		...
		bool OnUserUpdate(float fElapsedTime) override {
			...
			if(bRenderTris) {
				...
				for(auto i : tris)
					triPipeline(i);
				std::sort(storedTris.begin(), storedTris.end(),
