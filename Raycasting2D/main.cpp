...
constexpr auto map =
		std::experimental::fundamentals_v2::make_array<std::string_view>(
	"#####################################################################RRRR",
	"#   #               #               #           #                   #   R",
	"#   #               #               #           #                   #   R",
	"#   #               #               #           #                   #   R",
...
class Example : public olc::PixelGameEngine {
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		for(size_t x = 0; x < ScreenHeight() / 2; x++) {
			...
			if(iShade < 0)
				iShade = 0;
			DrawLine(0, x, ScreenWidth(), x,
					olc::Pixel(iShade, iShade, iShade));
			DrawLine(0, ScreenHeight() - x, ScreenWidth(),
					ScreenHeight() - x, olc::Pixel(iShade, iShade, iShade));
		}

		for(int iRayNum = 0; iRayNum < ScreenWidth(); iRayNum++) {
			float fRayDist = 0;
			const float fRayDir = fPlayerDir
			...
			if(fRayDist < fMaxDist) {
				...
				int iCeiling = (float)(ScreenHeight() / 2) - ScreenHeight()
					/ fRayDist;
				...
				DrawLine(iRayNum, iCeiling, iRayNum, iFloor,
