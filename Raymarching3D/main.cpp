	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		for(int iScreenX = 0; iScreenX < ScreenWidth(); iScreenX++) {
			for(int iScreenY = 0; iScreenY < ScreenHeight(); iScreenY++) {
				Ray ray{cam.pos, cam.pitch +
					atanf(((float)iScreenY / ScreenHeight() - 0.5f) * fFovY),
						cam.yaw + atanf(((float)iScreenX
									/ ScreenWidth() - 0.5) * fFovX)};
		...
		DrawString(0, 0, std::to_string(cam.pos.x) + ' '
				+ std::to_string(cam.pos.y) + ' '
				+ std::to_string(cam.pos.z), olc::WHITE);
		return true;
	}
};
