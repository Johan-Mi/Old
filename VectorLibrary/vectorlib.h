...
namespace vectorlib {
	...
	public:
		...
		Vector2 cross(Vector2 inVector) {
			return Vector2(x * (inVector.x + inVector.y),
				y * (inVector.x + inVector.y));
		}
		float compSum() {
			return x + y;
		}
	...
	public:
		...
		Vector3 cross(Vector3 inVector) {
			return Vector3(x * (inVector.x + inVector.y + inVector.z), y * (inVector.x + inVector.y + inVector.z), z * (inVector.x + inVector.y + inVector.z));
		}
		float compSum() {
			return x + y + z;
		}
